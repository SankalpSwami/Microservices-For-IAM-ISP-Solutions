    ...
    add: (req, res) => {
        mongoose.connect(connUri, { useNewUrlParser: true, useUnifiedTopology: true }, (err) => {
            ...
            if (!err) {
                ...
                const credits = '000'
                const user = new User({ username: username, password: password, useraccesslevel: useraccesslevel, uniqueid: uniqueid, registeredby: registeredby, credits: credits }); // document = instance of a model
